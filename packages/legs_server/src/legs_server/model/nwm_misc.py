...
# Load data_config from the bundled config directory
_config_dir = os.path.join(os.path.dirname(__file__), "config")
_data_config_path = os.path.join(_config_dir, "data_config.yaml")
if os.path.exists(_data_config_path):
    with open(_data_config_path, "r") as f:
        data_config = yaml.safe_load(f)
else:
    data_config = {"action_stats": {"min": [-2.5, -4], "max": [5, 4]}}
